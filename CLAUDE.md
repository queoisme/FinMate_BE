...
**Email OTP (Phase 17).** Partly reverses the 2026-09-11 decision to skip OTP — that one was phone plus SMS; this is email, which needs no carrier. It closes two holes: forgetting a password used to mean losing the account outright (only `change-password` existed, and it requires being signed in), and anyone could register with someone else's address. `users.email_verified_at` is backfilled to now for existing accounts, or enforcing verification would lock out every current user. `forgot-password` answers identically for a real and a made-up address — byte for byte — and swallows the cooldown branch too, because an anonymous endpoint that distinguishes them enumerates the user table. Codes are single-use, expire in ten minutes and die after five wrong guesses; the SHA-256 at rest only keeps them out of a dump or log, since six digits is 10⁶ preimages and falls in seconds either way. Codes are scoped by purpose, so a verification code cannot reset a password. Resets revoke every refresh token and reuse the registration password rules. Brevo is called with a plain `HttpClient` — no new package — and falls back to logging the code when `BREVO_API_KEY` is absent, which is how dev and tests read it. `REQUIRE_EMAIL_VERIFICATION` defaults on; `AuthApiFactory` turns it off because 23 test files register then sign in immediately, and `OtpAuthControllerTests` turns it back on through `WithWebHostBuilder` rather than touching the shared environment variable.

**Remaining:** nothing blocked; Core Flow 1 and Flow 2 are now complete server-side. Two tasks are deliberately skipped (phone/SMS OTP, the three `ab_*` tables). **Core Flow 3 and Core Flow 4 have never been checked against the docx** — Phase 11 only read Flow 1 and Flow 2 end to end, so the task count proves nothing about those two.
