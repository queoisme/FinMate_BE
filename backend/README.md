...
- Lịch theo **giờ VN (UTC+7) cố định** (`Application/Common/VietnamTime.cs` — dùng chung cho chu kỳ budget, biên ngày của `daily_summaries`, streak và chu kỳ mission), không dùng `TimeZoneInfo.FindSystemTimeZoneById` vì `InvariantGlobalization=true` bật solution-wide. Mốc chu kỳ luôn trả về ở **offset 0** — Npgsql từ chối ghi `DateTimeOffset` có offset khác 0 vào cột `timestamptz`, kể cả khi chỉ dùng làm tham số truy vấn. Cần năm/tháng để dựng cache key thì dùng `VietnamTime.YearMonthOf`, cần ngày thì dùng `VietnamTime.DateOf` — đừng đọc `.Year`/`.Month`/`.Day` của giá trị UTC.
